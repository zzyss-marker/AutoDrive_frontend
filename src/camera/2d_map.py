...
class RGBDFrame:
    ...
    # 创建Open3D点云
    def create_open3d_point_cloud(self, camera_intrinsics: dict) -> o3d.geometry.PointCloud:
...
# 点云转2D地图
def point_cloud_to_2d_map(points: np.ndarray, resolution: float = 0.05, width: int = 200, height: int = 200) -> np.ndarray:
...
# 将转换后的3D点投影到二终地图上
def update_global_map(global_map, transformed_points, resolution=0.05, width=200, height=200):
    origin_x, origin_y = width // 2, height // 2
...
def main():
    ...
    try:
        while True:
            ...
            if des is not None and prev_des is not None and len(des) > 0 and len(prev_des) > 0:
                ...
                if matches:
                    ...
                    # 使用深度信息转换为3D点
                        z = prev_frame.depth[int(prev_kp[obj_idx].pt[1]), int(prev_kp[obj_idx].pt[0])]
                        if z == 0:  # 深度为0表示无效点
                            continue
                    ...
                    if len(obj_points) >= 4 and len(img_points) >= 4:
                        ...
                        transformed_points = [camera_pose.transform_point(point) for point in obj_points]
